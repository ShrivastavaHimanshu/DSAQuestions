...
public class EquilibriumIndex {
	...
	public static int solve(int[] a) {
		...
		int n = a.length;
		System.out.println("lent is :" + n);
		long leftIndexCount = 0;
		long rightIndexCount = 0;
		int maxCount = Integer.MAX_VALUE;
		int ans = -1;
		for (int i = 0; i< n; i++) {
			if (i == 0) {
				leftIndexCount = 0;
				rightIndexCount= prefixSum[n-1]-prefixSum[0];
			} else if (i == (n - 1)) {
				leftIndexCount = prefixSum[n-2];
				rightIndexCount = 0;
			}else {
				// i = 0 1 2 3 4 5 6
				// a = 1 2 3 7 1 2 3
				// sum [(i+1) to (n-1)
				leftIndexCount = prefixSum[i-1];
				rightIndexCount = (prefixSum[n - 1] - prefixSum[i]);
			}
			System.out.print("leftIndes is :" + leftIndexCount );
			System.out.println(" " +"rightIndex is :" + rightIndexCount);
			if (leftIndexCount == rightIndexCount) {
				if (i < maxCount) {
					ans = i;
				}
			 maxCount = i;	
